...
class My_ElasticSearch:
    ...
    def __init__(self, es_url:str, index_name:str, index_file_path:str, api_key:str, create:bool=False):
    ...
    def create_index(self):
        ...
        if self.create == True or not self.es.indices.exists(index=self.index_name):
    ...
    ############################################################
    def update(self, esid, doc):

        body = {
            'doc': doc
        }

        res=self.es.update(index=self.index_name, id=esid, body=body)
        return res
